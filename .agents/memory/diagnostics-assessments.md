...
## Single-user
- Attempts have NO userId — the content model is single-user even though the site now sits behind required Google login (login gates access; it does not scope data). Don't add per-user scoping unless the whole app moves to multi-user.
